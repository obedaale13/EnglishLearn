...
public class HomeworkAdapter extends BaseAdapter<Homework> {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (null == view) {
            ...
            holer.layout_images = (RelativeLayout) view.findViewById(R.id.imagesLayout);
            view.setTag(holer);
        } else {
...
//       displayTextView(msg.getTitle(), holer.tv_title);
        holer.tv_content.setText(msg.getDetail());
        holer.tv_title.setText(msg.getTitle());
...
//显示发送时间
        ...
        if (null != msg.getImagePaths()){
            //显示图片
            loadImages(holer.layout_images,msg.getImagePaths());
        }


        holer.iv_head.setOnClickListener(new View.OnClickListener() {
            @Override
